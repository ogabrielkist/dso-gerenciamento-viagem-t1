...
class TelaCidadeGUI(TelaBaseGUI):
    ...
    def le_opcao(self) -> str | None:
        layout = [
            [sg.Text("Gerenciamento de Cidades", font=("Helvetica", 18), justification='center', expand_x=True)],
            [sg.Button("Incluir Cidade", key='-INCLUIR-', size=(20, 2), pad=(5, 5))],
            [sg.Button("Listar Cidades", key='-LISTAR-', size=(20, 2), pad=(5, 5))],
            [sg.Button("Editar Cidade", key='-EDITAR-', size=(20, 2), pad=(5, 5))],
            [sg.Button("Excluir Cidade", key='-EXCLUIR-', size=(20, 2), pad=(5, 5))],
            [sg.VPush()], 
    ...
    def pega_dados_entidade(self, lista_paises: list[Pais], dados_atuais: dict = None) -> dict | None:
        if not lista_paises:
            self.mostra_erro("Não é possível adicionar uma cidade pois não há países cadastrados.")
            return None
        ...
        titulo_janela = "Editar Cidade" if dados_atuais else "Incluir Nova Cidade"

        layout = [
            [sg.Text(titulo_janela, font=("Helvetica", 16))],
            [sg.Text("Nome:", size=(8, 1)), sg.Input(nome_atual, key='-NOME-')],
            [sg.Text("País:", size=(8, 1)), 
             sg.Combo(nomes_paises, default_value=pais_atual_nome, readonly=True, key='-PAIS-')],
    ...
    def mostra_lista_entidades(self, dados_lista: list):
        ...
        """
        if not dados_lista:
            self.mostra_mensagem("Aviso", "Nenhuma cidade cadastrada.")
            return

        headings = ["ID", "Nome", "País"] 
        
        data = [
            [d['id'], d['nome'], d['pais'].nome]
    ...
    def seleciona_entidade(self, dados_lista: list, titulo_janela: str) -> str | None:
        if not dados_lista:
            self.mostra_erro("Não há cidades cadastradas para selecionar.")
            return None
